...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http,  MvcRequestMatcher.Builder mvc ) throws Exception {
		...
		.authorizeHttpRequests(authorize -> authorize //1. 페이지 접근에 관한 설정
				...
				.requestMatchers(mvc.pattern("/myPage/**"),(mvc.pattern("/order/**")),(mvc.pattern("/cart/**"))).authenticated()
				.requestMatchers(mvc.pattern("/css/**"), mvc.pattern("/js/**"), mvc.pattern("/img/**"), mvc.pattern("/image/**"), mvc.pattern("/fonts/**"), mvc.pattern("/errorPage")).permitAll()
				...
				.requestMatchers(mvc.pattern("/category"),mvc.pattern("/item/**"),mvc.pattern("/profile/**"),mvc.pattern("/followReq")).permitAll()
